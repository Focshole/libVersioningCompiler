 ...
 */
#ifndef LIB_VERSIONING_LIBTOOL_COMPILER_SYSTEM_COMPILER_OPTIMIZER_CPP
#define LIB_VERSIONING_LIBTOOL_COMPILER_SYSTEM_COMPILER_OPTIMIZER_CPP

#include "versioningCompiler/CompilerImpl/LibToolSystemCompilerOptimizer.hpp"

namespace vc {

std::vector<void *> LibToolSystemCompilerOptimizer::loadSymbols(
    const std::filesystem::path &bin, const std::vector<std::string> &func,
    void **handler) {
  std::vector<void *> symbols = {};
  if (initialized) {
    if (exists(bin)) {
      *handler = lt_dlopen(bin.c_str());
    } else {
...
void LibToolSystemCompilerOptimizer::releaseSymbol(void **handler) {
